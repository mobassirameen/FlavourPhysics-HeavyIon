...
void bgdiscrimination()
{
  ...
  RooDataSet *data = new RooDataSet("data", "data", RooArgSet(*svmass, *mistag), Import(*chain_data));
    ...
    TH1F * mthistSB_II = (TH1F*)data_SBReg->createHistogram("mistag",nbins);
     TH1F * mthist_TEST= (TH1F*)data_TEST->createHistogram("mistag",nbins);
     TH1F *SignalMinusBG=(TH1F*)mthistSR->Clone();
     //mthistSB->Scale(integral_mass2->getVal()/integral_mass1->getVal());//28241.7/365090
    ...
    mthistSB_II->Scale(0.004);
    BGMinusSig->Add(mthistSB_II,-1);
    BGMinusSig->Sumw2();
    for (int itera1=0; itera1<nbins; itera1++)
    {
      ...
      SignalMinusBG->Draw();
      //TCanvas *lm = new TCanvas("lm","lm",600,600);
      //mthist_TEST->Draw();
    ...
    RooDataHist * bkgdatahist = new RooDataHist("bkgdatahist", "bkgdatahist", *mistag, Import(*BGMinusSig));
    ...
    RooRealVar *conts = new RooRealVar("conts","conts", 0.3,0.1, 0.5);
    ...
    RooRealVar *lambda = new RooRealVar("lambda", "lambda", 1.04492e-02, 0.0, 1);
    RooRealVar *gamma = new RooRealVar("gamma", "gamma", 2.00000e-02, 0.0, 0.1);
    RooRealVar *delta = new RooRealVar("delta", "delta", 7.33407e-01, 0, 2);
...
//===============================================================================================
  ...
  // RooProdPdf *bkgpdf = new RooProdPdf("bkgpdf", "mass*mistag",RooArgList(*expoBg,*kerestiMTBG));
   ...
   Double_t N_SIGErr_SR = nSig->getError()*iMT_sig->getVal();
    ...
    std::cout<<"========================="<<"\n";
    RooAbsReal* iMT_bkg_sb = bkgpdf->createIntegral(*mistag,NormSet(*mistag),Range("sbleft,sbright")) ;
