...
//=================================================================================================================Helicity Amplitudes
        h120s = abs((real(sleft))*(srQplus/(msquark-mupquark))*((mxi-msigma)*f1V + (q2/mxi)*f3V));
        h120p = abs((real(sleft))*(srQminus/(msquark+mupquark))*(-(mxi+msigma)*f1A + (q2/mxi)*f3A));
          h120v = abs((1.0)*(srQminus/sqrt(q2))*((mxi+msigma)*f1V + (q2/mxi)*f2V));
          h121v = abs((1.0)*(sqrt(2.0)*srQminus)*(-f1V - ((mxi+msigma)/mxi)*f2V));
          h12tv = abs((1.0)*(srQplus/sqrt(q2))*((mxi-msigma)*f1V + (q2/mxi)*f3V));
          h120a = abs((1.0)*(srQplus/sqrt(q2))*(-(mxi-msigma)*f1A + (q2/mxi)*f2A));
          h121a = abs((1.0)*(sqrt(2.0)*srQplus)*(f1A - ((mxi-msigma)/mxi)*f2A));
          h12ta = abs((1.0)*(srQminus/sqrt(q2))*(-(mxi+msigma)*f1A + (q2/mxi)*f3A));
...
//===============================================================================================================================================================
    ...
    helscalar0->Draw("same");
    helscalar0->SetMarkerStyle(23);
    helscalar0->SetMarkerColor(9);
    helpseudoscalar0->Draw("same");
    helpseudoscalar0->SetMarkerStyle(22);
    helpseudoscalar0->SetMarkerColor(9);
    TLegend *legend = new TLegend(0.01,0.4,0.014,0.5);
    legend->SetHeader("only g_{L}^{S}","C");
    ...
    legend->AddEntry(helscalar0, "H_{#frac{1}{2}0}^{S}");
    legend->AddEntry(helpseudoscalar0, "H_{#frac{1}{2}0}^{P}");
    legend->Draw();
    ...
    legend1->AddEntry(diffAsFq2, "Only g_{L}^{S}");
    ...
    legend2->AddEntry(AFBAsFq2, "Only g_{L}^{S}");
    ...
    legend3->AddEntry(ConAsFq2, "Only g_{L}^{S}");
    ...
    legend4->AddEntry(HadPolAsFq2, "Only g_{L}^{S}");
    ...
    legend5->AddEntry(BRFracAsFq2, "Only g_{L}^{S}");
